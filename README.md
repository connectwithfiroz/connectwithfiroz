# Hi 👋 I'm Firoz Ansari

### About Me
Full-stack web developer (PHP / Laravel) and product maker. I build web apps and side-products like QuizSagar, Qnote and TadkaZone. Passionate about clean code, scalable systems, and practical AI integration.

- 🔭 I’m currently building: **Tutizo** (tutor-student matchmaking) and improving **Qnote**.
- 🌱 Learning: React, AI/ML integration, and modern DevOps practices.
- 💼 Experience: 3+ years as Laravel Full Stack Developer.
- 📫 Reach me: **contacttofiroz@gmail.com** | [LinkedIn](https://www.linkedin.com/in/hifiroz)

### 🛠️ Tech & Tools
**Languages:** PHP, JavaScript, SQL, Python  
**Frameworks:** Laravel, Bootstrap, Vue.js (learning React)  
**Databases:** MySQL, PostgreSQL, MongoDB  
**DevOps & Tools:** Git, Docker, GitHub Actions, cPanel

### ⭐ Highlights
- Built and shipped **QuizSagar** and **TadkaZone** (restaurant digital menu & ordering).
- Experience building school management & complaint management systems.
- Comfortable with end-to-end product development: idea → launch → iteration.

### 🔭 Featured projects
- **QuizSagar** — Online quiz platform. *(Pinned repo: `quizsagar`)*  
- **Qnote** — Note taking app with YouTube note features. *(Pinned repo: `qnote`)*  
- **TadkaZone** — Digital menu and ordering for restaurants. *(Pinned repo: `tadkazone`)*  
*(Add repo links and short one-line descriptions for each pinned project.)*

### 📈 Github Stats
<!-- You can paste GitHub stats cards here (optional) -->
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=connectwithfiroz&layout=compact)
![Stats](https://github-readme-stats.vercel.app/api?username=connectwithfiroz&show_icons=true)

### 🤝 Let’s connect
If you like my work or want to collaborate — open an issue on any repo, or drop an email: **contacttofiroz@gmail.com**
